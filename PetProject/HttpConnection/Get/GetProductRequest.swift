...
protocol GetProductRequestProtocol {
    func response(product: Product?, getProduct status: String)
}


class GetProductRequest: HttpRequest {
    
    // MARK: Property
    var delegate: GetProductRequestProtocol?
    let apiUrl = API_URL + "/get/product"
    ...
    func fetch(vc: UIViewController, isShowAlert: Bool = true, paramDict: [String: String]) {
        ...
        guard let encodedUrlString = urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else {
            ...
            delegate?.response(product: nil, getProduct: "ERR_URL_ENCODE")
            return
        }
        
        guard let url = URL(string: encodedUrlString) else {
            if isShowAlert { vc.showErrorAlert(title: "ERR_URL") }
            delegate?.response(product: nil, getProduct: "ERR_URL")
            return
        }
        ...
        let task = URLSession(configuration: conf).dataTask(with: httpRequest, completionHandler: { (data, response, error) in DispatchQueue.main.async {
            ...
            if let _ = error {
                ...
                self.delegate?.response(product: nil, getProduct: "ERR_SERVER")
                return
            }
            
            guard let res = response as? HTTPURLResponse else {
                if isShowAlert { vc.showErrorAlert(title: "ERR_RESPONSE") }
                self.delegate?.response(product: nil, getProduct: "ERR_RESPONSE")
                return
            }
            
            if res.statusCode != 200 {
                if isShowAlert { vc.showErrorAlert(title: "ERR_STATUS_CODE") }
                self.delegate?.response(product: nil, getProduct: "ERR_STATUS_CODE")
                return
            }
            
            guard let data = data else {
                if isShowAlert { vc.showErrorAlert(title: "ERR_DATA") }
                self.delegate?.response(product: nil, getProduct: "ERR_DATA")
                return
            }
            
            guard let status = self.getStatusCode(data: data) else {
                if isShowAlert { vc.showErrorAlert(title: "ERR_STATUS_DECODE") }
                self.delegate?.response(product: nil, getProduct: "ERR_STATUS_DECODE")
                return
            }
            
            if status != "OK" {
                if isShowAlert { vc.showErrorAlert(title: status) }
                self.delegate?.response(product: nil, getProduct: status)
                return
            }
            
            // MARK: Response
            do {
                let decoded = try JSONDecoder().decode(ProductRequestResult.self, from: data)
                let resProduct = decoded.result
                ...
                self.delegate?.response(product: product, getProduct: "OK")
                
            } catch {
                if isShowAlert { vc.showErrorAlert(title: "ERR_DATA_DECODE", message: "데이터 응답 오류가 발생했습니다.") }
                self.delegate?.response(product: nil, getProduct: "ERR_DATA_DECODE")
            }
        }})
        task.resume()
    }
